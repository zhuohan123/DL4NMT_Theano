...
def zoneout_layer(state_before, last_state, use_noise, trng, zoneout_rate = 0.):
    """Zoneout"""
    if zoneout_rate > 0.0:
        noise = trng.binomial(state_before.shape, p=(1. - zoneout_rate), n=1,
                              dtype=state_before.dtype)
        projection = T.switch(
            use_noise,
            state_before * noise + last_state * (1.0 - noise),
            state_before * (1.0 - zoneout_rate) + last_state * zoneout_rate
        )
    else:
...
def zoneout_pack(step_fn, len_seqs, trng, zoneout_rate_h, zoneout_rate_c):
    def new_fn(*all_inputs):
        step_fn_inputs = all_inputs[:-1]
        use_noise = all_inputs[-1]
        ret = step_fn(*step_fn_inputs)
        last_h = all_inputs[len_seqs]
        last_c = all_inputs[len_seqs + 1]
        h, c = ret[0], ret[1]
        h = zoneout_layer(h, last_h, use_noise=use_noise, trng=trng, zoneout_rate=zoneout_rate_h)
        c = zoneout_layer(c, last_c, use_noise=use_noise, trng=trng, zoneout_rate=zoneout_rate_c)
        return (h, c) + ret[2:]
...
def lstm_layer(P, state_below, O, prefix='lstm', mask=None, **kwargs):
    ...
    zoneout_params = kwargs.pop('zoneout_params', None)  # (use_noise, trng, zoneout_rate_h, zoneout_rate_c)
    ...
    if zoneout_params is not None:
        _step = zoneout_pack(_step, len(seqs), *zoneout_params[1:])
        shared_vars.append(zoneout_params[0])
